    ...
    if (rank == 0) {
        ...
        if (A->nb_line % numproc != 0) {
            for (int i = numproc; i >= 2; i--) {
                MPI_Send(linearize_colonne + (A->nb_colon * r * (i)), r * A->nb_colon, MPI_LONG, (rank + 1) % numproc,
                         99, MPI_COMM_WORLD);
            }
        } else {
            for (int i = numproc; i >= 2; i--) {
                MPI_Send(linearize_colonne + (A->nb_colon * r * (i - 1)), r * A->nb_colon, MPI_LONG,
                         (rank + 1) % numproc, 99, MPI_COMM_WORLD);
            }
        }
...
void scatter_line(Matrix A, int send_count, long *recv_data_line, int recv_count) {
     ...
     */
        ...
        if (A->nb_line % numproc != 0) {
            for (int i = numproc; i >= 2; i--) {
                MPI_Send(linearize_lines + (A->nb_colon * r * (i)), r * A->nb_colon, MPI_LONG, (rank + 1) % numproc, 99,
                         MPI_COMM_WORLD);
            }
        } else {
            for (int i = numproc; i >= 2; i--) {
                MPI_Send(linearize_lines + (A->nb_colon * r * (i - 1)), r * A->nb_colon, MPI_LONG, (rank + 1) % numproc,
                         99, MPI_COMM_WORLD);
            }
        }
    } else {
...
void gather(long *send_data, int send_count, long *recv_data, int recv_count, int int_size_of_mat) {
    ...
    if (rank == 0) {
        ...
        for (i = 0; i < numproc - 1; i++) {
            MPI_Recv(recv_data + int_size_of_mat - (recv_count * (i + 1)), recv_count, MPI_LONG, numproc - 1, 99,
    ...
    for (int i = 0; i < prev->nb_line; i++) {
        for (int j = 0; j < prev->nb_colon; j++) {
            if (i == rank_of_prev_mat + 1 || j == rank_of_prev_mat + 1) {
                ...
                to_ret->tab[i][j] = minimum(prev->tab[i][j],
                                            prev->tab[i][rank_of_prev_mat] + prev->tab[rank_of_prev_mat][j]);
            }
        }
    }
    return to_ret;
...
int main(int argc, char *argv[]) {
    ...
    scatter_line(A, 0, save_line, recieved_count);
    scatter_colum(A, 0, save_colum, recieved_count);
